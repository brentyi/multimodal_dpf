    ...
    def __init__(self, *paths, **kwargs):
        ...
        for trajectory in trajectories:
            ...
            for t in range(0, timesteps):
                ...
                for i in range(100):
                    std_dev = np.array([0.1, 0.01])
                    if i < 60:
                        noisy_state = state + \
                            np.random.normal(
                                loc=0., scale=std_dev, size=state.shape)
                    else:
                        noisy_state = state + \
                            np.random.normal(
                                loc=0., scale=std_dev * 5, size=state.shape)

                    log_likelihood = np.asarray(scipy.stats.multivariate_normal.logpdf(
                        noisy_state, mean=state, cov=std_dev ** 2))
                    dataset.append(
                        torch_utils.to_torch((noisy_state, observation, log_likelihood)))

        self.dataset = dataset
        print("Loaded {} points".format(len(dataset)))
